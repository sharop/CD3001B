...
obs_pred_df_raw <- 
  function(y,cutoff,XGB_Matrix,XGB){
...
make_roc_table <- function(model, 
  ...
  upper_limit <- max(predict(model, data) %>% unique)
  
  lower_limit <- min(predict(model, data) %>% unique)
  ...
  for(cutoff in intervals){
    ...
    y_yhat <- obs_pred_df_raw(y=yflag,cutoff=cutoff,XGB_Matrix=data,XGB=model)
  ...
  return (
    tibble(
      threshold, TP, FP, FN, TN, N, precision, TPR_recall, FPR, lift
    )
  )
...
make_shap_viz <- function(dataz, finalXGB, yvar, shap_viz_name, ...){
  ...
  total_length <- max(shap_viz[[1]]$feature_value) - min(shap_viz[[1]]$feature_value)
  ...
  shap_viz <-
    xgb.ggplot.shap.summary(
      ...
      data = dataMatrix) +
    theme_ipsum() +
    scale_color_gradientn(colours = blues,
